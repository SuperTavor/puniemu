...
using Puniemu.Src.Server.GameServer.Requests.Game.GameStart.DataClasses.WibWob;
using Puniemu.Src.Server.GameServer.Requests.UpdateTutorialFlag.DataClasses.WibWob;
using Supabase.Realtime.Converters;
namespace Puniemu.Src.Server.GameServer.Requests.Game.GameStart.Logic.WibWob
{
    public static class GameStartHandler
    {
        ...
        public static async Task HandleAsync(HttpContext ctx)
        {
            ...
            var stageRoot = JsonConvert.DeserializeObject<JObject>(
                DataManager.Logic.DataManager.GameDataManager!.GamedataCache["ywp_mst_stage"]!
            );

            var stagesInfo = stageRoot!["data"]!.ToObject<List<StageItem>>(); 
            ...
            var tutorialList = await UserDataManager.Logic.UserDataManager.GetYwpUserAsync<List<Tutorial>>(deserialized!.Gdkey!, "ywp_user_tutorial_list");
            ...
            StageItem currentStageInfo;
            try
            {
                currentStageInfo = stagesInfo.Where(x => x.StageID == deserialized.StageId).First();
            }
            ...
            if (LevelData.TutorialEdit != null && LevelData.TutorialEdit.TutorialReq != null)
            {
                foreach (TutorialEntry item in LevelData.TutorialEdit.TutorialReq)
                {
                    if (item.FirstClear == 0 || (item.FirstClear == 1 && res.IsFirstClear == 1))
                    {
                        ...
                        try
                        {
                            tuto = tutorialList.Where(x => x.TutorialId == item.TutorialId && x.TutorialType == item.TutorialType).First();
                        }
                        catch(InvalidOperationException)
                        {
                            tutorialList.Add(new Tutorial()
                            {
                                ...
                                TutorialStatus = item.TutorialStatus,
                                TutorialType = item.TutorialType
                            });
                            tuto = tutorialList.Last();
                        }
                        
                        //Set the tutorial status
                        tuto.TutorialStatus = item.TutorialStatus;
                    }
                }
            }
            ...
            res.StageType = currentStageInfo.StageType; 
            res.BattleType = currentStageInfo.StageObjectType; //posibly??
