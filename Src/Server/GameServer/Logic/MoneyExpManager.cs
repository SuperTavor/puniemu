...
namespace Puniemu.Src.Server.GameServer.Logic
{
    public class MoneyExpManager
    {
        ...
        */
        ...
        public static UserYoukaiResultListRes GiveYoukaiExp(
                YwpUserYoukai youkai,
                long youkaiId,
                int expToAdd,
                TableParser<YwpMstYoukai> YoukaiMstTable,
                TableParser<YwpMstYoukaiLevel> YoukaiLevelMstTable,
                TableParser<YwpMstYoukaiLevelOpen> YoukaiLevelOpenMstTable)
        {
            var YoukaiMstIndex = MstYoukaiManager.GetYoukaiIndex(YoukaiMstTable, youkaiId);
            if (YoukaiMstIndex < 0)
                throw new Exception($"Unknown yokai {youkaiId}");

            UserYoukaiResultListRes result = new();

            result.HaveFlg = false;
            result.CanEvolve = false;
            result.IsLockLevel = false;
            result.isMaxLevel = false;
            result.youkaiId = youkaiId;
            ...
            if (youkai.Level < YoukaiMstTable.Items[YoukaiMstIndex].MaxLevel &&
                youkai.IsLockedLevel == 0)
            {
                ...
                while (levelIndex != -1)
                {
                    levelIndex = MstYoukaiManager.GetYoukaiLevelIndex(
                        YoukaiLevelMstTable,
                        YoukaiMstTable.Items[YoukaiMstIndex].LevelType,
                        level);
                ...
                int levelOpenIndex = MstYoukaiManager.GetYoukaiLevelOpenIndex(
                    ...
                    result.Before.Level,
                    result.After.Level,
                    YoukaiMstTable.Items[YoukaiMstIndex].YoukaiRarity);
            ...
            int hpOffset =
                (YoukaiMstTable.Items[YoukaiMstIndex].MaxHp -
                 YoukaiMstTable.Items[YoukaiMstIndex].BaseHp)
                / YoukaiMstTable.Items[YoukaiMstIndex].MaxLevel;

            int atkOffset =
                (YoukaiMstTable.Items[YoukaiMstIndex].MaxAtk -
                 YoukaiMstTable.Items[YoukaiMstIndex].BaseAtk)
                / YoukaiMstTable.Items[YoukaiMstIndex].MaxLevel;
            ...
            if (youkai.Level >= YoukaiMstTable.Items[YoukaiMstIndex].MaxLevel)
            {
                ...
                youkai.IsLockedLevel = 0;
                youkai.Level = YoukaiMstTable.Items[YoukaiMstIndex].MaxLevel;
            }
