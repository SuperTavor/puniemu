...
initGachaReqPayload = f'{{"activeDeckId":1,"appVer":"{appver}","deviceId":"d-ug1ysfdpazslszazpcqvs60n2xwwgb2xjr02j7h9j7s10sy1pgg2yrr1kq2lhc","level5UserId":"g-tnfy8487sh1ztm3tro8v6k034bdesydrke05ooxc67zp0sy1z162yrs9mq2lhg","mstVersionVer":MstVersionPlaceholder,"osType":2,"token":"d4cdab206b50dd37155dce81d35edbfb67f012082f5fea9d70e21dc7e36a013b","userId":"3327355726187","ywpToken":"0"}}'
...
try:
    ...
    #Replace mst version in all other requests
    initGachaReqPayload = initGachaReqPayload.replace("MstVersionPlaceholder", str(init_dict["mstVersionMaster"]))
    ...
    decryptedLogin = decrypt_res(encryptedLoginRes.text)
    login_dict = json.loads(decryptedLogin)
    ...
    # get some from init gacha 
    initGachaRes = requests.post(GAMESERVER+"initGacha.nhn",headers=headers,data=encrypt_req(initGachaReqPayload))
    decGacha = decrypt_res(initGachaRes.text)
    with open("dbg.txt","w",encoding='utf-8') as f: f.write(decGacha)
    gachaDict = json.loads(decGacha)
    for key,value in gachaDict.items():
        if "ywp_mst" in key and key not in table_trigger or key == "gachaStampList" or key == "gachaStampIdList" or key == "gachaStampRewardList" or key == "canPossessionItemList" or key == "gachaLotRuleList" or key == "canUseMultiGachaCoinIdList" or key == "bannerResourceList":
            if key not in table_trigger:
                table_trigger.append(key)
                json.dump(value,open(os.path.join(output_folder, f"{key}.txt"), "w",encoding="utf-8"),ensure_ascii=False)
        elif "ywp_user" in key:
